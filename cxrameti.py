...
class NineteenCommands(commands.Cog):
    def __init__(self, bot):
        self.bot = bot

    @app_commands.command(name="regchannel_19_00", description="დაარეგისტრირე არხი 19:00 როლით")
    ...
    async def regchannel_19_00(self, interaction: discord.Interaction, channel: discord.TextChannel, role_19_00: discord.Role, banned_role: discord.Role, teamlist_channel: discord.TextChannel):
    ...
    @app_commands.command(name="reg_19_00", description="გამოაგზავნე რეგისტრაციის შეტყობინება")
    @app_commands.checks.has_permissions(administrator=True)
    async def reg_19_00(self, interaction: discord.Interaction):
        member = await check_user_permissions(interaction, 1368589143546003587, 1005186618031869952, interaction.guild_id)
        if not member:
            return
    ...
    @app_commands.command(name="createteamlist_19_00", description="შექმნის Team List 19:00")
    @app_commands.checks.has_permissions(administrator=True)
    async def createteamlist_19_00(self, interaction: discord.Interaction):
        try:
            ...
            if not record or "registered_messages_19:00" not in record:
                await interaction.followup.send("⚠️ ჯერ არავინ არ არის დარეგისტრირებული 19:00-ზე.", ephemeral=True)
                return
    ...
    @app_commands.command(name="clearlist_19_00", description="წაშალე Team List 19:00")
    @app_commands.checks.has_permissions(administrator=True)
    async def clearlist_19_00(self, interaction: discord.Interaction):
        member = await check_user_permissions(interaction, 1368589143546003587, 1005186618031869952, interaction.guild_id)
        if not member:
            return
...
async def setup(bot):
    await bot.add_cog(NineteenCommands(bot))
